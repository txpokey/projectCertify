...
import sci.category.certify.service.bootstrap.constants.AdhocSqlStatements
...
//    @Transactional
    void createTablesManually() {
        ...
        doShowTables()
        runSqlExpectingVoidReturned(client, AdhocSqlStatements.CREATE_TABLE_USING_H2_CONSOLE_CODE_EXAMPLE)
        runSqlExpectingSingleAnswer(client, AdhocSqlStatements.SELECT_COUNT_ALL_TABLES_FROM_META_TABLE , "C")
        runSqlExpectingListAsAnswer(client, AdhocSqlStatements.SELECT_ALL_TABLES_FROM_META_TABLE)
        runSqlExpectingListAsAnswer(client, AdhocSqlStatements.SELECT_TABLES_AND_COUNTS_FROM_META_TABLE)
        false
//        showConversionFluxToIterableOnAllPrimesRxInDatabase()
    }

    void doAutocommitSettings() {
        def what = runSqlExpectingVoidReturned(client, AdhocSqlStatements.SET_AUTOCOMMIT)
        what
    }
    void doShowTables() {
        def what = runSqlExpectingListAsAnswer(client, AdhocSqlStatements.SHOW_TABLES)
        what
    }
